...
import MiniCart from "../../../Pages/MiniCart/MiniCart.Layout";
import { useAppDispatch, useAppSelector } from "../../../Hooks/hooks";
import Login from "../../../Pages/Login/Login.Layout";
import { useAuth } from "../../../Context/AuthContext";
import { getCartItemList } from "../../../Pages/MiniCart/MiniCartSlice";
const Header = () => {
  ...
  const getCurrentLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        //maps.googleapis.com/maps/api/geocode/json?latlng=40.714224,-73.961452&sensor=true&key=YOUR_KEY
        // var requestOptions = {
        //   method: "GET",
        // };

        fetch(
          `https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${long}&apiKey=9a114045c13c4a6bb35f379f238ceefa`
        )
  ...
  return (
    <>
      <HeaderContainer>
        ...
        <AddressContainer onClick={getCurrentLocation}>
          <h2>Delivery in 13 minutes</h2>
          <div>
            <AddressText>{`${location?.address_line1} ${location?.address_line2}`}</AddressText>
            <div></div>
          </div>
        </AddressContainer>
      ...
      {isModalOpen && (
        <Modal location="right" close={handleCloseModal}>
      ...
      {isLoginOpen && (
        <Modal location="center" height="auto" close={handleLoginCloseModal}>
          <h2>Login</h2>
          <button onClick={handleLoginCloseModal}>Close</button>
          <Login closeHandler={handleLoginCloseModal} />
        </Modal>
      )}
    </>
  );
